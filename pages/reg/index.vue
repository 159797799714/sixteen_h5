      ...
      <view class="form m-lg bg-panel-3 p-lg rounded-md">
        ...
        <view class="form-item border-b m-b-md p-b-xs">
          ...
          <v-input class="color-light" v-model="form.code" :placeholder="$t('reg.a7')">
            <template #right>
              <!-- 手机 -->
              <v-code v-show="form.type == 1" url="/register/sendSmsCode" :data="{
                  phone: form.account,
                  country_code: country_code,
                  type: 1
                }"></v-code>
              <!-- 邮箱 -->
              <v-code url="/register/sendEmailCode" :data="{
                  email: form.account,
                  type: 1
                }" v-show="form.type == 2"></v-code>
            </template>
          </v-input>
        </view>
        <view class="form-item border-b m-b-md p-b-xs">
        ...
        <view class="form-item border-b m-b-md p-b-xs">
          ...
          <v-input class="color-light" v-model="form.password_confirmation" type="password"
            :placeholder="$t('reg.b1')"></v-input>
        </view>
        <view class="form-item border-b m-b-md p-b-xs">
          <view class="label m-b-xs">{{ $t('reg.b2') }}</view>
          <v-input class="color-light" v-model="form.invite_code" :placeholder="`${$t('reg.b3')}000000`"></v-input>
        </view>
        <v-button type="blue" block class="w-max m-b-md rounded-md" @click="register">{{ $t('reg.b9') }}</v-button>
        <view class="tips d-flex m-t-md fn-sm">
          <van-checkbox :value="check" @change="check = !check" class="flex-shrink" icon-size="15px"
            checked-color="#f05319"></van-checkbox>
          <view class="m-l-xs">
            {{ $t('reg.b5') }}
            <v-link class="color-sell" v-if="agreement.agreement"
              :to="{ path: '/pages/notice/detail', query: { id: agreement.agreement.id } }">《{{ agreement.agreement.title }}》</v-link>{{ $t('reg.b7') }}
            <v-link class="color-sell" v-if="agreement.clause"
              :to="{ path: '/pages/notice/detail', query: { id: agreement.clause.id } }">《{{ agreement.clause.title }}》</v-link>
          </view>
        </view>
      </view>
      <view class="d-flex justify-between m-lg">

          <view class="item transition-3 p-y-sp-r-sm p-x-xs fn-16" :class="{
              'color-sell': form.type == 2
            }" @click="form.type = 2">{{ $t('reg.a1') }}
            <view class="w-5 h-5 rounded-max bg-light justify-center mauto m-y-sm" :class="{
              'disnone': form.type == 1
            }">

            </view>
          </view>
          <view v-if="true" class="item transition-3 p-y-sp-r-sm p-x-xs fn-16 " :class="{
              'color-sell': form.type == 1
            }" @click="form.type = 1">{{ $t('reg.a0') }}
            <view class="w-5 h-5 rounded-max bg-light justify-center mauto m-y-sm" :class="{
              'disnone': form.type == 2
            }">
            </view>
