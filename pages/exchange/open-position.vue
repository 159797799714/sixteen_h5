<template>
  <view>
	...
	<popup :isShow='showInt' @close='showInt = false' @input="intPass"  @confirm='confirm'/>
  </view>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    //     .then(() => {
		...
		let isSet = JSON.parse(uni.getStorageSync('user')).is_set_payword
		console.log(isSet)
		if(isSet == 0){
			uni.showToast({
				title:this.$t('login.c2'),
				icon:"none"
			})
			setTimeout(()=>{
				uni.navigateTo({
					url:"/pages/safe/transaction-password"
				})
			},2000)
			return;
		}
	...
	confirm(){
		if(this.form.pay_password == ''){
			uni.showToast({
				title:this.$t('login.c1'),
				icon:"none"
			})
			return ;
		}
		
		this.openPosition(this.curSide);
		this.showInt = false
